...
def register_draft_content(llm: Any, drafts_dir: Path | str) -> None:
    """Register the DraftContent local function tool."""
    llm.register_local_function(
        "DraftContent",
        make_draft_content_handler(llm, drafts_dir),
        ...
        description=(
            ...
            "opening or to continue existing text (Kimi Partial Mode). Paste "
            "the draft via the clipboard hint instead of re-reading it."
        ),
    )
