...
# ---------------------------------------------------------------------------
# Overlap dedup: IoU > 5% keeps only the higher-score box (no nesting exempt)
# ---------------------------------------------------------------------------

def test_overlapping_boxes_keep_only_higher_score(fake_yolo):
    # A (0,0)-(100,100), B (50,50)-(150,150): inter=2500, union=17500,
    # IoU ~ 0.143 > 0.05 -> B dropped.
    FakeYOLO.boxes = [
        ([0, 0, 100, 100], 0.9),
        ([50, 50, 150, 150], 0.7),
    ]
    det = yd.YoloDetector("model.pt")
    anns = det.detect(_image(1000, 1000))

    assert len(anns) == 1
    assert anns[0]["score"] == pytest.approx(0.9)


def test_slightly_overlapping_boxes_both_kept(fake_yolo):
    # A (0,0)-(100,100), B (95,0)-(195,100): inter=500, union=19500,
    # IoU ~ 0.026 <= 0.05 -> both kept.
    FakeYOLO.boxes = [
        ([0, 0, 100, 100], 0.9),
        ([95, 0, 195, 100], 0.7),
    ]
...
def test_nested_boxes_deduplicated_like_any_overlap(fake_yolo):
    # Nesting is NOT exempt: inner (50,50)-(100,100) inside outer (0,0)-(200,200)
    # gives IoU = 2500/40000 = 0.0625 > 0.05 -> inner dropped.
    FakeYOLO.boxes = [
        ([0, 0, 200, 200], 0.9),
        ([50, 50, 100, 100], 0.7),
    ]
...
def test_tiny_box_inside_huge_box_kept_when_iou_low(fake_yolo):
    # (0,0)-(1000,1000) vs (10,10)-(30,30): IoU = 400/1e6 ~ 0.0004 -> both kept.
    FakeYOLO.boxes = [
        ([0, 0, 1000, 1000], 0.9),
        ([10, 10, 30, 30], 0.7),
    ]
...
def test_transitive_overlap_keeps_non_conflicting_boxes(fake_yolo):
    # B overlaps A (dropped); C overlaps neither A nor B -> A and C survive.
    FakeYOLO.boxes = [
        ([0, 0, 100, 100], 0.9),
        ([60, 0, 160, 100], 0.8),
        ([200, 0, 300, 100], 0.7),
...
def test_labels_renumbered_after_dedup(fake_yolo):
    FakeYOLO.boxes = [
        ([0, 0, 100, 100], 0.95),
        ([10, 10, 90, 90], 0.8),   # IoU 0.64 with the first -> dropped
        ([300, 300, 400, 400], 0.7),
    ]
    det = yd.YoloDetector("model.pt")
    anns = det.detect(_image(1000, 1000))

    assert [a["label"] for a in anns] == [1, 2]
    assert anns[0]["score"] == pytest.approx(0.95)
    assert anns[1]["score"] == pytest.approx(0.7)
