...
> **目标项目**：Caelum-Agent-Pro（基于 Microsoft UFO2 二次开发，融入 Agent-S2 规划机制，适配 Kimi K2.6 API）
...
7. [Kimi K2.6 适配方案](#7-kimi-k26-适配方案)
8. [实现路线图](#8-实现路线图)
...
- **Kimi K2.6 独家适配**：Formula 内置工具注册/执行、Files API 三种 purpose、Partial Mode、JSON Mode、熔断器等全部对齐
...
| Kimi API 的 model 名拼写容错 | `kimi-k2-6→kimi-k2.6` 这种坑已踩过 | 配置校验沿用 |
...
| **LLM 后端** | Kimi K2.6 深度适配 | GPT-4o/o1 通用 | 可插拔（OpenAI/Anthropic/HF TGI） | **Kimi K2.6 保持深度适配** |
...
class HostAgentProcessorWithPHP(HostAgentProcessor):
    ...
    async def _php_refine_plan(self, original_instruction, completed,
        ...
        # 用 Kimi K2.6 调用，return 更新的 remaining 列表
...
- [ ] 验证：用 `kimi-k2.6` 跑通 UFO2 自带的 demo（Notepad 打字、文件管理器操作）
...
| Kimi K2.6 在 UFO2 的 JSON 结构化输出上不如 GPT-4o | 中 | 中 | 加容错解析（`_parse_verdict` 风格）；关键路径用 `response_format` 强制 JSON |
