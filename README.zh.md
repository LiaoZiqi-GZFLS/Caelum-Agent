...
- **多模态感知：** 截图 + OCR + 无障碍树 + YOLO（OmniParser）的 SoM 标注。
- **浏览器自动化：** 走 Playwright MCP，无障碍树优先交互。
- **桌面自动化：** 走 Windows-MCP，UIA/A11y 优先，必要时回退到坐标/图像方式。
- **Kimi K3 大脑：** OpenAI 兼容 API，支持 Formula 工具与本地函数工具。
...
- Windows 10/11
- Python **3.12**（`windows-mcp` 要求 `>=3.12`）
...
视觉标注需要 OmniParser `icon_detect` YOLO 权重，默认放在 `./models/omniparser/icon_detect/`（约 40MB）。从 GitHub Release 镜像下载：
...
- `yolo`：OmniParser YOLO 模型路径、device、置信度、图像尺寸、自动补偿开关。
...
├── ui_detector/               # OmniParser YOLO + SoM 标注
├── skills/                    # SKILL.md 技能库
...
截图
    ...
    ├── UIA/A11y 控件树 ──┐
    │                       ├── 结构化环境描述 → Kimi
    └── YOLO 图标检测（无 UIA 的界面）→ SoM 标注 ──┘
```

### 并发模型

- 主循环：asyncio
- IO 线程池：最多 8 个 worker（截图、OCR、YOLO 检测、文件 IO、MCP I/O）
...
- 虚拟环境**只能用 Python 3.12**（`windows-mcp` 要求 `>=3.12`）。
- **唯一的本地模型是 OmniParser YOLOv8 图标检测器**，经 ultralytics 加载（不用 Ollama/GGUF/vLLM）。权重（约 40MB）用 `python setup.py --download-weights` 获取。
